...
class CurrentConditionsViewModel @Inject constructor(private val api: OpenWeatherMapApi): ViewModel() {
    ...
    suspend fun fetchCurrentLocationData(latitudeLongitude: LatitudeLongitude) {
        val currentConditions = api.getCurrentConditions1(latitudeLongitude.latitude, latitudeLongitude.longitude)
        _currentConditions.trySend(currentConditions)
    }
}